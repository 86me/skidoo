...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Skidoo is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Skidoo is downloading blocks...");
...
Value getauxblock(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Skidoo is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Skidoo is downloading blocks...");
