...
// Copyright (c) 2013-2014 Megacoin Developers/"Kimoto Chan"
// Copyright (c) 2014 The Justified Ancients of Mummu
...
const string strMessageMagic = "Skidoo Signed Message:\n";
...
int64 GetMinFee(const CTransaction& tx, bool fAllowFree, enum GetMinFee_mode mode)
{
    ...
    int64 nMinFee = (1 + (int64)nBytes / 1000000) * nBaseFee;
...
static const int64 nStartSubsidy = 0 * COIN;
static const int64 nMinSubsidy = 0 * COIN;

int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = nStartSubsidy;
	if(nHeight%2 == 0) {nSubsidy = .00000001 * COIN;}
    else if (nHeight%3 == 0)  {nSubsidy = .0000001 * COIN;}
    else if (nHeight%5 == 0)  {nSubsidy = .000001 * COIN;}
    else if (nHeight%17 == 0) {nSubsidy = .00001 * COIN;}
    else if (nHeight%23 == 0) {nSubsidy = .00001 * COIN;}
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2.3 * 60 * 25; 
static const int64 nTargetSpacing = 2.3 * 60; 
static const int64 nInterval = nTargetTimespan / nTargetSpacing; 
static const int64 nAveragingInterval = nInterval * 3; 
static const int64 nAveragingTargetTimespan = nAveragingInterval * nTargetSpacing; 

static const int64 nMaxAdjustDown = 23;
static const int64 nMaxAdjustUp = 123; 
...
unsigned int static KimotoGravityWell(const CBlockIndex* pindexLast, const CBlockHeader *pblock, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
 ...
 {
         static const int64          BlocksTargetSpacing                          = 2.3 * 60; 
...
{
		 int DiffMode = 1;
		 if (pindexLast->nHeight+1 >= 230) { DiffMode = 2; }
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SkidooMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SkidooMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running SkidooMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
